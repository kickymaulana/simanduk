...
const props = defineProps<{ proses: { id: number; proses: string }[]; rows: { nama: string; hari: Record<string, number>; total: number; persentase: number }[]; output: number; bulan: number; tahun: number; proses_id: number | null; days: number }>();
const bulan = ref(String(props.bulan));
const tahun = ref(String(props.tahun));
const prosesId = ref(String(props.proses_id ?? ""));
let timeout: ReturnType<typeof setTimeout>;
watch([bulan, tahun, prosesId], () => { clearTimeout(timeout); timeout = setTimeout(() => router.get(route("laporan.kerusakan.qc"), { bulan: bulan.value, tahun: tahun.value, proses_id: prosesId.value }, { preserveState: true, replace: true }), 250); });
